...
const getExpenseByCategory = asyncHandler(async (req, res) => {
  const data = await analyticsService.getExpenseByCategory(req.user.id);
...
const getWeeklySpending = asyncHandler(async (req, res) => {
  const data = await analyticsService.getWeeklySpending(req.user.id);
...
const getCashFlow = asyncHandler(async (req, res) => {
  const data = await analyticsService.getCashFlowTrend(req.user.id);
...
const getSavingsGrowth = asyncHandler(async (req, res) => {
  const data = await analyticsService.getSavingsGrowth(req.user.id);
